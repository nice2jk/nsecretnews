...
<head>
	...
	<script type="text/javascript">
		...
		function getContents() {
			$.ajax({
	        	type: "GET",
	        	url: "soccerepl?category=${category}&offset=${offset}&search=${search}",			
		...
		function viewHtml(data) {
            ...
            for(i = 0; i < data.contentList.length; i++) {
                ...
                writehtml += "<span class='font-italic'>" + data.contentList[i].league + "</span>";
                writehtml += "<span class='text-success'>" + data.contentList[i].mtime + "</span>";
                writehtml += "</div>";
		...
		function goSubmit() {
			...
			if(!searchKey) {
				...
				$.ajax({
		        	type: "GET",
		        	url: "soccerepl.do?category=${category}&offset=${offset}&search=" + searchKey,			
		            success:function(data){
		            	offset = data.offset;
...
<body>
	...
	<nav aria-label="pagination">
	  <ul class="pagination justify-content-center">
	  		...
	  		<c:otherwise>
		  		<c:choose>
					<c:when test="${empty search }">
						<li class="page-item"><a class="page-link" href="/soccerepl.do?category=${category}&offset=${offset-20}">< 다음</a></li>
					</c:when>
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="/soccerepl.do?category=${category}&offset=${offset-20}&search=${search}">< 다음</a></li>
					</c:otherwise>
		  		</c:choose>
	  		</c:otherwise>
	  	</c:choose>
	    ...
	    <c:choose>
	    	<c:when test="${empty search }">
	    		<li class="page-item"><a class="page-link" href="/soccerepl.do?category=${category}&offset=${offset+20}">이전 ></a></li>
	    	</c:when>
	    	<c:otherwise>
	    		<li class="page-item"><a class="page-link" href="/soccerepl.do?category=${category}&offset=${offset+20}&search=${search}">이전 ></a></li>
	    	</c:otherwise>
	    </c:choose>	    
	  </ul>
	</nav>
	
	<form class="form-inline justify-content-center" action="/soccerepl.do" name="searchForm">	  
